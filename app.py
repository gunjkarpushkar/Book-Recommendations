...
def recommend_categories():
    ...
    if categories_name:
        recommended_books = dt[dt['categories'] == categories_name][['title', 'authors', 'categories']].head(10)
        return render_template('categories.html', recommended_books=recommended_books.to_html(index=False))
    else:
        return "No category selected."

# @app.route('/recommend_categories', methods=['GET', 'POST'])
# def recommend_categories():
#     if request.method == 'POST':
#         categories_name = request.form.get('categories_name')
#         if categories_name:
#             recommended_books = dt[dt['categories'] == categories_name][['title', 'authors', 'published_year']].head(10)
#             return render_template('categories.html')
#         else:
#             return "No category selected."
